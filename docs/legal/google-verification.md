...
Owner runbook for getting Agad verified for the `gmail.send` scope. Built from a
2026 research pass against Google's official docs.

> **First, a caveat on the legal pages.** The Privacy Policy (`/privacy`) and Terms
> (`/terms`) shipped in M4 are accurate to what Agad does, but they are **templates,
...
## What Agad already provides (built in M4)
...
> The product was renamed to **Agad** before any of this was created. The OAuth client, the domain
> and the hosting app do not exist yet, so claim all three under **Agad** and never under the old name.

1. **Register a domain** you own (not GitHub Pages / Notion / Google Sites — Google must be
   able to verify ownership).
2. **Deploy Agad on it over HTTPS** (M5/deploy) so `/`, `/privacy`, `/terms` are
...
- User type: **External**
- App name: **Agad** (must match the homepage branding exactly — the pages say **Agad**,
  never the old name, so type it exactly like that)
- App logo
...
1. Record an **unlisted YouTube** demo video (English, < 5 min) that shows:
   - the OAuth consent flow with **"Agad"** visible on the consent screen,
   - the browser **address bar showing your OAuth client ID** (Google asks for this),
   - the app **actually using the scope** — i.e. Agad sending a tailored application
...
> Agad is a job-application assistant for onlinejobs.ph job seekers. When a new job
> matching the user's saved keywords is posted, Agad generates a tailored application and
...
> scope: Agad never reads, lists, or alters the user's existing mail. The OAuth refresh
